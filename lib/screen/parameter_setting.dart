...
class _ParameterSettingScreenState extends State<ParameterSettingScreen> {
  ...
  void initState() {
    super.initState();
    intensity = Utils.getDeviceIntensity();
    volumeController = TextEditingController();
    volumeController.text = Utils.getDeviceVolume().toString();
  ...
  Widget build(BuildContext context) {
    double spaceHeight = (MediaQuery.of(context).size.height - 620) / 2;
    return Scaffold(
                          ...
                          onPressed: () async {
                            if (!isLoading) {
                              if (int.parse(volumeController.text) >= Constants.VOLUME_MIN && int.parse(volumeController.text) <= Constants.VOLUME_MAX) {
                                ...
                                ToastUtils.showSuccessToast(context, "Intensity and volume have been saved successfully");
                              } else {
                                String volumeError = "";
